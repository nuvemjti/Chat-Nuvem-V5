...
import { initRouteTracking } from "./utils/routePersistence";
...
const App = () => {
  ...
  useEffect(() => {
    // Inicializa o rastreamento de rotas
    const cleanup = initRouteTracking();
...
useEffect(() => {
  ...
  const handleBeforeUnload = () => {
    if (userId) {
      // Adiciona o atraso de 1 minuto (60.000 ms)
      setTimeout(() => {
        ...
        .then((response) => {
          if (response.ok) {
            ...
            //console.error("Falha ao marcar como offline.");
          }
        })
        .catch((error) => console.error("Erro ao marcar como offline:", error));
      }, 60000); // 1 minuto de atraso
    }
  };

  // Adiciona o evento para quando o usuário tenta fechar o navegador
