...
const ShowTicketLogModal = ({ isOpen, handleClose, ticketId }) => {
  ...
  const typeDescriptions = {
    ...
    nps: i18n.t("showTicketLogModal.options.nps"), //Nome integrado ao catalogo de tradução. --> Igor
    autoClose: i18n.t("showTicketLogModal.options.autoClose") //Nome integrado ao catalogo de tradução. --> Igor
                ...
                <Step key={index}>
                  <StepLabel>
                    {`${log.type === 'access' ||
                      ...
                      log.type === 'open' || 
                      log.type === 'pending' ||                       
                      log.type === 'nps' || //adicionado para conter texto valido dentro do log --> Igor 
                      log.type === "closed" ||
                      log.type === 'autoClose' || //adicionado para conter texto valido dentro do log --> Igor 
                      log.type === "reopen"
                      ? log?.user?.name
                    ...
                    ${typeDescriptions[log.type]} - ${format(
                            ...
                            'dd/MM/yyyy HH:mm:ss' //Adicionado segundos para melhor referencia --> Igor
                          )}`}
                  </StepLabel>
                </Step>
              ))}
