    ...
    <Paper elevation={0} variant="outlined" className={classes.ticketsWrapper}>
      ...
      <Paper square elevation={0} className={classes.ticketOptionsBox}>
        <Grid container alignItems="center" justifyContent="space-between">
          ...
          <Grid item>
  {/* Flexbox para colocar o checkbox ao lado do dropdown */}
      <div style={{ display: 'flex', alignItems: 'center' }}>
        <Badge
          invisible={!isHoveredCheckbox} // Tornar invisível até que o mouse esteja sobre o checkbox
          badgeContent={i18n.t("Todas as filas")}
          classes={{ badge: classes.tabsBadge }}
        >
          <Checkbox
            checked={selectedQueueIds.length === user?.queues?.length}
            indeterminate={
              selectedQueueIds.length > 0 &&
              selectedQueueIds.length < user?.queues?.length
            }
            onChange={handleSelectAllQueues}
            onMouseEnter={() => setIsHoveredCheckbox(true)} // Ao passar o mouse
            onMouseLeave={() => setIsHoveredCheckbox(false)} // Ao remover o mouse
            style={{
              cursor: "pointer",
              height: 30,
              width: 30,
            ...
            color="primary" // Garantir que o Checkbox use a cor primária do tema
          />
        </Badge>

        {/* Dropdown para selecionar as filas */}
        <TicketsQueueSelect
