...
const FlowBuilderAddQuestionModal = ({
  ...
  const { updatedVariables } = useContext(FlowVariablesContext)
  ...
  const handleSavePrompt = (values) => {
    ...
    if (open === "edit") {
      ...
      const variables = values.answerKey
      if(!variables){
        return toast.error("Crie ao menos uma variável");
      }
      ...
      updatedVariables(values.answerKey)
