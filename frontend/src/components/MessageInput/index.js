...
const useStyles = makeStyles((theme) => ({
  ...
  emojiBox: {
    ...
    zIndex: 9999, // Garante que fique sobreposta aos outros elementos
  },
  circleLoading: {
    color: green[500],
    opacity: "70%",
...
const MessageInput = ({
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setSchedulesEnabled(planConfigs.plan.useSchedules);
    }
    fetchData();
  ...
  const handleQuickAnswersClick = async (value) => {
    if (value.mediaPath) {
      try {
        ...
        // Aqui você pode configurar como enviar o anexo
        handleUploadQuickMessageMedia(data, value.value); // Envia a mídia e a mensagem
                  ...
                  <ClickAwayListener onClickAway={(e) => setShowEmoji(false)}>
                    <Picker
                      ...
                      i18n={{
                        search: "Procurar",
                        clear: "Limpar",
                        notfound: "Esse emoji não foi encontrado.",
                        categories: {
