...
const Reports = () => {
  ...
  const [selectedTagKanban, setSelectedTagKanban] = useState([]);
  ...
  const handleFilter = async (pageNumber) => {
    ...
    try {
      const data = await getReport({
        ...
        contactId: selectedContactId,
        whatsappId: JSON.stringify(selectedWhatsapp),
        tags: JSON.stringify(selectedTagKanban),
        users: JSON.stringify(userIds),
        queueIds: JSON.stringify(queueIds),
        status: JSON.stringify(selectedStatus),
        // tags: tagIds,
  ...
  const handleSelectedTags = (selecteds) => {
    const tagKanban = selecteds.map((t) => t.id)
    setSelectedTagKanban(tagKanban)
  }
