    ...
    if (user.companyId) {
      ...
      const onCompanyChatPopover = (data) => {
        if (data.action === "new-message") {
          ...
          const userInChat = data.chat.users.some(
            (chatUser) => chatUser.userId === user.id,
          );

          // Se o usuário não for o remetente e estiver no chat, toca o som
          if (userInChat && data.newMessage.senderId !== user.id) {
            soundAlertRef.current(); // Toca o som
          }
        }
