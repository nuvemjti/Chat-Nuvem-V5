...
const LoggedInLayout = ({ children, themeToggle }) => {
 ...
 //   getSetting();
  ...
  useEffect(() => {
  console.log("🟢 Children recebidos pelo LoggedInLayout:", children);
}, [children]);  
  ...
  <>
    {React.Children.toArray(children).map((child, index) => {
      if (typeof child === "string") {
        ...
        if (!trimmed || trimmed === ";") {
          console.warn(`⚠️ String inválida removida: "${child}"`);
          return null;
        }
      }
      ...
      console.warn("🧽 Ignorado conteúdo não reconhecido:", child);
      return null;
    })}
  </>
)}
