...
import ExtractIcon from "@material-ui/icons/GetApp";
...
const useStyles = makeStyles((theme) => ({
  listItem: {
    ...
    "&:hover $iconHoverActive": {
      ...
      boxShadow: `0 0 8px 3px ${theme.palette.primary.main}80`, // Efeito de luz ao redor
      transition:
        "background-color 0.3s ease, color 0.3s ease, box-shadow 0.3s ease", // Transição suave
      color: "#fff",
    },
  },
  ...
  return (
    <div onClick={drawerClose}>
      ...
      {showKanban && planExpired && (
        <Can
          role={
            user.profile === "user" && user.allowKanban === "enabled"
              ? "admin"
              : user.profile
          }
      ...
      {planExpired && (
        <Can
          role={
            user.profile === "user" && user.allowContacts === "enabled"
              ? "admin"
              : user.profile
          }
          perform="dashboard:view"
        ...
        yes={() => (
          <>
            ...
            {planExpired && (
              <Can
                role={
                  user.profile === "user" && user.allowCampaigns === "enabled"
                    ? "admin"
                    : user.profile
                }
                perform="dashboard:view"
            ...
            {/* Botão de Extração de Leads temporariamente desativado
            {user.super && (
              <ListItemLink
                to="/extracao"
                primary="Extração de Leads"
