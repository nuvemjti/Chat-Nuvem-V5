...
const verifyQueue = async (
  ...
  if (queues.length === 1 && !chatbot) {
    ...
    if (greetingMessage.length > 1 && sendGreetingMessageOneQueues) {
      // ✅ Verifica se a saudação já foi enviada antes de continuar
      ...
      if (existingMessage) {
        console.log("Saudação já enviada, ignorando duplicação.");
        return;
      }
  ...
  const botText = async () => {
    ...
    if (choosenQueue || (queues.length === 1 && chatbot)) {
      ...
      // Verificação para enviar a mensagem de "opção inválida" apenas uma vez
      ...
      // Verifica se há mídia anexada no ticket
      if (ticket.whatsapp.greetingMediaAttachment !== null) {
        ...
        if (fileExists) {
          ...
          const debouncedSentgreetingMediaAttachment = debounce(
            async () => {
              ...
              // Atualiza o ticket com a quantidade usada do bot
              await UpdateTicketService({
                ticketData: {
                  amountUsedBotQueues: ticket.amountUsedBotQueues + 1
                },
          ...
          // Se o arquivo não existir, apenas envia a mensagem de texto
...
const withMedia = (mensagem: proto.IWebMessageInfo): boolean => {
  const hasMedia =
    ...
    mensagem.message?.ephemeralMessage?.message?.viewOnceMessage?.message
      ?.imageMessage ||
    mensagem.message?.ephemeralMessage?.message?.viewOnceMessage?.message
      ?.videoMessage ||
    mensagem.message?.ephemeralMessage?.message?.viewOnceMessage?.message
    ...
    mensagem.message?.templateMessage?.hydratedFourRowTemplate
      ?.documentMessage ||
    ...
    mensagem.message?.highlyStructuredMessage?.hydratedHsm?.hydratedTemplate
      ?.documentMessage ||
    mensagem.message?.highlyStructuredMessage?.hydratedHsm?.hydratedTemplate
      ?.videoMessage ||
    mensagem.message?.highlyStructuredMessage?.hydratedHsm?.hydratedTemplate
...
const isMsgForwarded = (mensagem: proto.IWebMessageInfo): boolean => {
  const forwarded =
    mensagem.message?.extendedTextMessage?.contextInfo?.isForwarded ||
...
const handleMessage = async (
  ...
  try {
    ...
    if (
      (msg.key.fromMe && /\u200e/.test(bodyMessage)) ||
      (!hasMedia &&
        msgType !== "conversation" &&
        msgType !== "extendedTextMessage" &&
        msgType !== "contactMessage" &&
        ...
        if (enableLGPD && ticket.status === "lgpd" && !isImported) {
          if (
            ...
            if (
              ...
              choosenOption <= 0 ||
              (choosenOption !== 1 && choosenOption !== 2) // Verifica se a opção não é 1 ou 2
            ) {
              const invalidOptionMessage =
                "📜 Você precisa aceitar os termos para que possamos continuar o atendimento.\n" +
                "✅ Caso queira prosseguir, envie uma nova mensagem e selecione a opção correta.";
