...
export default async function CreateMessageService({
  ...
  const completeMediaPath = mediaPath
    ? mediaPath.startsWith("http")
        ? mediaPath
        : `${process.env.BACKEND_URL.replace(/\/$/, "")}/public/chats/${mediaPath.replace(/^\//, "")}`
    : null;

console.log("Complete Media Path:", completeMediaPath);
