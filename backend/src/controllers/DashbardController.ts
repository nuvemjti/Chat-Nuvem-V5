...
export const monitoringMetrics = async (req: Request, res: Response): Promise<Response> => {
  const { companyId } = req.user;
  
  try {
    const metrics = await DashboardDataService(companyId, {});
    return res.status(200).json(metrics);
  } catch (error) {
    return res.status(500).json({ error: "Erro ao buscar métricas de monitoramento" });
  }
};