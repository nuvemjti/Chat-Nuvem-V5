...
async function handleRandomUser() {
  const jobR = new CronJob('*/30 * * * * *', async () => {
    try {
      const companies = await Company.findAll({
        attributes: ['id', 'name'],
        where: { status: true },
          ...
          c.queues.map(async (q) => {
            ...
            if (count > 0) {
              for (const ticket of tickets) {
                const { queueId, userId, id: ticketId } = ticket;
                ...
                // Verifica se a mensagem de saudação já foi enviada
                const greetingMessageExists = await Message.findOne({
                  where: {
                    ticketId,
                ...
                // Função para verificar se o usuário é do perfil 'user'
                const isUserProfile = async (userId) => {
                  const user = await User.findOne({ where: { id: userId } });
                  return user?.profile === "user";
                };

                // Enviar mensagem de saudação após 30 segundos, se necessário
                if (sendGreetingMessageOneQueues && !greetingMessageExists) {
                ...
                // Aguardar o tempo configurado em tempoRoteador antes de atribuir o atendente
                const tempoRoteadorMilliseconds = tempoRoteador * 60 * 1000; // tempoRoteador em minutos convertido para milissegundos
                const currentTime = moment().utc();
                const ticketUpdatedAt = moment(ticket.updatedAt).utc();
                const elapsedTime = currentTime.diff(ticketUpdatedAt); // Tempo passado desde a última atualização do ticket

                // Se o tempo passado desde a última atualização for maior que o tempoRoteador, atribui um atendente
                if (elapsedTime >= tempoRoteadorMilliseconds) {
                  // Se não houver atendente designado
                  if (!userId) {
                    const onlineUserIds = await Promise.all(
                      userIds.map(async (id) => (await isUserOnline(id) && await isUserProfile(id)) ? id : null)
                    ).then((ids) => ids.filter((id) => id !== null));

                    const randomUserId = getRandomUserId(onlineUserIds);
                    ...
                    if (randomUserId) {
                      ...
                      logger.info(`Ticket ID ${ticket.id} transferido para UserId ${randomUserId}`);
                    }
                  } else if (userIds.includes(userId)) {
                    const availableUserIds = userIds.filter((id) => id !== userId);

                    const onlineUserIds = await Promise.all(
                      availableUserIds.map(async (id) => (await isUserOnline(id) && await isUserProfile(id)) ? id : null)
                    ).then((ids) => ids.filter((id) => id !== null));
...
export async function startQueueProcess() {
  ...
  userMonitor.process("VerifyLoginStatus", async (job) => {
  const { userId, action } = job.data; // Passa dados sobre o login/logout

  if (action === 'login') {
    await handleUserLogin(userId);  // Marca o usuário como online
  } else if (action === 'logout') {
    await handleUserLogout(userId);  // Marca o usuário como offline
  }
});
